...
const AddReimbursementModal = ({
  ...
  handleSubmit,
  handleCloseModal,
  handleAddReimbursement, // Added handler to pass new data to parent
}) => {
  const [errors, setErrors] = useState({});
  const projectOptions = [
    { name: "Shriram Smash Hit", distance: 20 },
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:8080/reimbursement", dataToSubmit, {
        headers: { 'Content-Type': 'application/json' },
      });
      alert("Reimbursement added successfully!");
      handleAddReimbursement(response.data);  // Add new reimbursement to parent state
  ...
  useEffect(() => {
    if (formData.purpose === "Petrol") {
      setFormData((prevData) => ({ ...prevData, projects: [] }));  // Reset projects if needed
    }
  }, [formData.purpose]);
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-white p-8 rounded-lg w-3/4 md:w-1/2">
        <div className="flex justify-between items-center mb-4">
          <h5 className="text-xl font-semibold">Add Reimbursement</h5>
          <button onClick={handleCloseModal} className="bg-red-500 text-white p-2 rounded-full">
            <CgClose />
          </button>
        </div>
        ...
        <form onSubmit={handleFormSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <FormControl required error={!!errors.purpose} fullWidth>
              ...
              {errors.purpose && <span className="text-sm text-red-600">{errors.purpose}</span>}
            </FormControl>
            ...
            {formData.purpose === "Petrol" && (
              <>
                ...
                <FormControl required error={!!errors.projects} fullWidth>
                  ...
                  <Select name="projects" value={formData.projects} onChange={handleInputChange} multiple>
                    {projectOptions.map((project) => (
                      <MenuItem key={project.name} value={project.name}>
                        {project.name}
                      </MenuItem>
                  ...
                  {errors.projects && <span className="text-sm text-red-600">{errors.projects}</span>}
                </FormControl>
